        ...
        return {
          ...
          link: function (scope, element, attrs, ctrl) {
            ...
            callbacks.dropped = function (event) {
              ...
              var parent_id ;
              if(event.dest.nodesScope.$parent.$modelValue == null){
                parent_id = 0;
              }else{
                parent_id = event.dest.nodesScope.$parent.$modelValue.id;
              }

              //code for sorting data
              var siblingsLength = event.dest.nodesScope.$modelValue.length;

              //get the div with the menu ID
              var getMenuId = event.dest.nodesScope.$parent.$element.context.attributes['dataValue'].value;
              
              //update sortOrder in database
              for(var i = 0; i < siblingsLength; i++){
                //update Dragged Value in database
                scope.updateNew(event.dest.nodesScope.$modelValue[i], parent_id, getMenuId, i+1)
              }
            };
